...
// From src/components/LandingPage.tsx, assets are ONE level up
...
function Badge({
  ...
  return (
    <span className={["inline-flex items-center rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wider", variants[variant]].join(" ")}>
      {children}
    </span>
  )
}
...
// ProjectCard
// -----------------------------------------------------------------------------
function ProjectCard({ title, desc, status = "Selected", href }: { title: string; desc: string; status?: string; href: string }) {
  return (
    <a
      href={href}
      target="_blank"
    ...
    >
      ...
      <div className="mt-8 flex items-center gap-2 text-sm font-bold text-slate-400">
        Open artifact <ChevronRight size={16} className="transition-transform group-hover:translate-x-1" />
      </div>
    </a>
  )
}
...
// WritingCard
// -----------------------------------------------------------------------------
function WritingCard({ title, desc, href, type }: { title: string; desc: string; href: string; type: "Medium" | "Case Study" | "Market Analysis" }) {
  const icon = type === "Medium" ? <BookOpen size={18} /> : type === "Case Study" ? <CheckCircle2 size={18} /> : <BarChart3 size={18} />
...
function CertificateCard({
  title, issuer, date, image, onOpen,
}: {
  title: string; issuer: string; date: string; image?: string; onOpen?: () => void
}) {
  return (
    <div
      onClick={onOpen}
    ...
    >
      ...
      {image && (
        <div className="ml-auto flex items-center gap-1 text-xs font-semibold text-slate-400">
      ...
      <div className="relative mx-auto mt-24 w-[92%] max-w-2xl">
        <div className="rounded-[2rem] border border-slate-200 bg-white shadow-2xl overflow-hidden">
          <div className="flex items-start justify-between gap-4 px-8 pt-8 pb-4">
            ...
            <button onClick={onClose} className="h-11 w-11 rounded-full border border-slate-200 bg-white text-slate-500 hover:text-slate-900 transition-colors flex items-center justify-center" aria-label="Close">
              <X size={18} />
            </button>
          </div>
...
// CertLightbox
// -----------------------------------------------------------------------------
function CertLightbox({ image, title, onClose }: { image: string; title: string; onClose: () => void }) {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-200 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative z-10 mx-4 w-full max-w-4xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 text-center text-sm font-semibold text-white/80">{title}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="fixed top-20 md:top-24 inset-x-0 z-40 overflow-hidden border-b border-slate-100 bg-white/80 backdrop-blur-md py-2">
      <div className="flex w-max animate-ticker gap-12 whitespace-nowrap">
        {looped.map((item, i) => (
          <span key={i} className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-slate-400">
            {item}
            <span className="h-1 w-1 rounded-full bg-slate-300" />
          </span>
        ))}
...
export default function LandingPage() {
  ...
  const GOOGLE_FORM_URL = "https://forms.gle/XijEstbTTTCAtJd49"
  ...
  const PROJECT_LINKS = {
    autoAnalyst: "https://autoanalyst-ai-powered-business-int.vercel.app/",
    growthEngine: "https://growth-intelligence-engine-beige.vercel.app/",
    goQuant: "https://github.com/Gurucharan87318/GoQuant_Latency-Topology-Visualizer.git",
    aiCodeGen: "https://github.com/Gurucharan87318/Ai-Code-Gen-V1.git",
  } as const

  const WRITING_LINKS = {
    medium1: "https://medium.com/@gurucharan87318/why-modern-bi-tools-still-fail-decision-makers-feea5c43ba08",
    medium2: "https://medium.com/@gurucharan87318/building-autoanalyst-turning-raw-data-into-board-ready-strategy-e51e8f5d392d",
  ...
  // TODO: Image tuning
  // ---------------------------------------------------------------------------
  const IMG_TUNE = { translateX: 10, translateY: 10, scale: 0.90 } as const
  const IMG_STAGE = { heightMobile: 500, heightDesktop: 520 } as const
  // ---------------------------------------------------------------------------

  const projects = [
    { title: "AutoAnalyst", status: "Flagship", href: PROJECT_LINKS.autoAnalyst, desc: "AI-Assisted Business Intelligence (Offline-First MVP). Reduces repetitive analytics work — exports, cleaning, chart building, and summary writing — so analysts can focus on decisions instead of dashboards." },
    { title: "Growth Intelligence Engine", status: "Selected", href: PROJECT_LINKS.growthEngine, desc: "A full-stack analytical platform bridging fragmented business metrics and executive action. Automates synthesis of market data, growth cohorts, and unit economics into decision-ready strategy memos." },
    { title: "GoQuant Latency Visualizer", status: "Selected", href: PROJECT_LINKS.goQuant, desc: "A premium trading dashboard with real-time latency visualization, advanced telemetry, authentication, and AI-powered assistant modules. GPT4All models used for data refinement and prompt chaining." },
    { title: "AI Code Generator v1.0", status: "Selected", href: PROJECT_LINKS.aiCodeGen, desc: "Transforms natural language prompts into clean HTML, CSS, and JavaScript. Built with FastAPI and React + Vite, integrating Together AI API for intelligent code generation." },
  ] as const

  const writings = [
    { type: "Medium" as const, title: "Why Modern BI Tools Still Fail Decision-Makers", href: WRITING_LINKS.medium1, desc: "We live in a world with more data than ever. Yet many organizations still struggle with slow decision cycles — and the tools designed to solve this sometimes make it worse." },
    ...
    { type: "Medium" as const, title: "Simulating an E-commerce Dataset to Learn Product Analytics", href: WRITING_LINKS.medium3, desc: "One of the challenges when learning analytics is access to real data. Most companies don't share internal datasets, so you need to create your own realistic environment." },
    { type: "Medium" as const, title: "Building a Product Analytics System Instead of Another Dashboard", href: WRITING_LINKS.medium4, desc: "Most data projects stop at a dashboard. But in real companies, dashboards are not the goal. Decisions are." },
  ] as const

  const certificates = [
    { title: "Business Value Creation", issuer: "Coursera - Duke University", date: "Feb 2026", image: certBusiness },
    { title: "Cash Flow Management", issuer: "Coursera - Duke University", date: "Aug 2024", image: certCashFlow },
    { title: "GenAI Job Simulation", issuer: "BCG - Forage", date: "Feb 2026", image: certBCG },
    { title: "GenAI-Powered Data Analytics", issuer: "Tata Group - Forage", date: "June 2025", image: genai },
  ] as const
          ...
          <div className="hidden md:flex items-center gap-1 rounded-full border border-slate-200/60 bg-white/55 p-1 shadow-sm backdrop-blur-md">
            {navItems.map((item) => (
              <button
                ...
                className="rounded-full px-5 py-2 text-sm font-medium transition-all text-slate-600 hover:text-slate-900 hover:bg-white/70 hover:backdrop-blur-md hover:shadow-sm active:scale-[0.98]"
              >
                {item.label}
              </button>
            ))}
            ...
            <div className="relative z-10 order-3 lg:col-span-6 lg:pb-4 pointer-events-auto">
              <Reveal delay={200}>
                <p className="mb-8 text-lg font-medium leading-relaxed text-slate-500 drop-shadow-sm">
                  Strategy, analysis, and systems thinking — grounded in computer science. I bridge the gap between data and decisions,
                  helping teams move from insight to action. I collaborate closely to design simple systems that actually improve outcomes.
                </p>
                <button
                  onClick={() => scrollToSection("contact")}
            ...
            <Reveal>
              <div className="mb-12">
                ...
                <p className="mt-3 text-slate-500 font-medium max-w-2xl">
                  I focus on decision-making, strategy mapping, and problem-solving.
                </p>
              </div>
            </Reveal>
          ...
          <div className="mx-auto max-w-350">
            <Reveal>
              <div className="mb-12">
                ...
                <p className="mt-3 text-slate-500 font-medium max-w-2xl">Writing is how I clarify thinking.</p>
              </div>
            </Reveal>
                ...
                <Reveal delay={120}>
                  <div className="rounded-[2rem] border border-slate-100 bg-white p-8 shadow-sm">
                    ...
                    <h2 className="mt-3 text-3xl md:text-4xl font-bold tracking-tight text-slate-900 leading-tight">
                      CS → Strategy.<br />Business is the interface.
                    </h2>
                    <p className="mt-6 text-slate-600 leading-relaxed font-medium">
                      I started in computer science because I loved building systems. Over time, I realized the hardest part isn't code — it's deciding what to build, what to ignore, and how to make decisions under constraints.
                    </p>
                    <p className="mt-5 text-slate-600 leading-relaxed font-medium">
                      I'm drawn to business problems where clarity is missing: messy data, unclear priorities, slow execution. I focus on reducing noise, making trade-offs explicit, and shipping work that improves real outcomes.
                    </p>
                    <p className="mt-5 text-slate-600 leading-relaxed font-medium">
                      What I optimize for: decision velocity, trust in outputs, and simple workflows that people actually use.
                    </p>
                  </div>
            ...
            <Reveal>
              <div className="text-center">
                <h3 className="text-3xl md:text-4xl font-semibold tracking-tight text-white">
                  Have a great idea in mind?<br />lets make it real
                </h3>
              ...
              <div className="md:col-span-6">
                <Reveal delay={100}>
                  ...
                  <div className="mt-5 flex items-center gap-3">
                    <a href={LINKEDIN_URL} target="_blank" rel="noreferrer" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10 transition-colors" aria-label="LinkedIn">
                      in
                    </a>
                    <a href={GITHUB_URL} target="_blank" rel="noreferrer" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10 transition-colors" aria-label="GitHub">
                      <Github size={18} />
                    </a>
                    <a href={`mailto:${EMAIL}`} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10 transition-colors" aria-label="Email">
                      <Mail size={18} />
                    </a>
                  </div>
                  <div className="mt-6">
                ...
                <Reveal delay={150}>
                  <div className="flex flex-col items-start md:items-end gap-3 text-sm text-slate-400">
                    <div className="flex flex-wrap gap-x-6 gap-y-2">
                      {navItems.map((i) => (
                        <button key={i.id} onClick={() => scrollToSection(i.id)} className="hover:text-white transition-colors">
                          {i.label}
                        </button>
                      ))}
                    </div>
